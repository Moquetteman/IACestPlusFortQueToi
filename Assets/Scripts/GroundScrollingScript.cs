...
public class ScrollingScript : MonoBehaviour
{
	...
	void Update()
	{
		...
		int width = (int)GetComponent<Renderer>().bounds.size.x;
		
		transform.name= width.ToString();
		//si la droite de la texture atteint le bord gauche de la camera
		if (this.transform.position.x < camLeftBorder-width/2) {
			

			this.transform.Translate(width*2,0,0);
			//float a = 57.6f-8.58864f;
			//transform.position.Set(a,0,0);
		} 
